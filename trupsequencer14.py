...
if not os.path.isdir("Resultslog/ResultslogGirsa2" + strFeatures):
    os.makedirs("Resultslog/ResultslogGirsa2" + strFeatures)
if not os.path.isdir("Modelslogs"):
    os.makedirs("Modelslogs")
if not os.path.isdir("Modelslogs/ModelslogGirsa2" + strFeatures):
    os.makedirs("Modelslogs/ModelslogGirsa2" + strFeatures)

def logmessage(s):
    with open("Resultslog/ResultslogGirsa2" + strFeatures + '/ResultslogGirsa2' + strFeatures + ".txt", "a") as w:
        w.write(str(s))
    print(s)
...
def CalculateLossForParagraph(para, fValidation=False):
    ...
    for iTrop, (trop, pos, tr, lex, hv, part, func, typ) in enumerate(para):
        ...
        if fValidation:
            ...
            output_trop.append((lex, trop, trop_vocab.getItem(predicted_iTrop)))
...
def run_network_on_validation(data_to_run, suffix):
    ...
    with codecs.open("Resultslog/ResultslogGirsa2" + strFeatures + '/ResultslogGirsa2' + strFeatures + str(suffix) + '.txt', mode='w', encoding='utf-8') as f:
...
def loadStopList():
    stopList = set() # type: Set[str]
    # type: () -> Set[str]
    f = open('DifferentCantillation.txt', 'r')
    for line in f: # type: str
        stopList.add(line.strip()) # get rid of \n
    f.close()
    return stopList

stopList = loadStopList()

def divideData(stopList, data):
    # type: (Set[str], List[Tuple[Any]]) -> Tuple[List[Tuple[Any], List[Tuple[Any]]]]
    training = [] # type: List[Tuple[Any]]
    testing = []  # type: List[Tuple[Any]]
...
all_data = list(read_data())
training, testing  = divideData(stopList, all_data)
#random.shuffle(all_data)
...
for epoch in range(30):
    ...
    model.save("Modelslogs/ModelslogGirsa2" + strFeatures + "/rnn_trup_e" + str(epoch) + '_loss' + str(average_loss) + '_acc' + str(average_acc) + '.model')
